...
# # - 큰 이동량 감지 시 해당 프레임 저장 (../output/issue)
# # ------------------------------------------------------------
...
#     large_motion_px: float = 100.0,         # ← 완화 (20 → 70)
...
#     ),  # ← 수렴 조건 완화
#     frame=None                               # ✅ 현재 프레임 전달 (for large motion save)
...
#             # ✅ large motion 프레임 저장
#             try:
#                 os.makedirs("../output/issue", exist_ok=True)
#                 if frame is not None:
#                     vis = frame.copy()
#                     next_pts_draw = next_pts[valid]
#                     prev_pts_draw = prev_pts[valid]
#                     for (p1, p2) in zip(prev_pts_draw, next_pts_draw):
#                         cv2.arrowedLine(
#                             vis, tuple(p1[0].astype(int)), tuple(p2[0].astype(int)),
#                             (0, 0, 255), 1, tipLength=0.3
#                         )
#                     name = f"large_motion_{int(time.time())}_{mean_flow:.1f}px.jpg"
#                     cv2.putText(vis, f"Large motion {mean_flow:.1f}px",
#                                 (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
#                                 0.8, (0, 0, 255), 2)
#                     cv2.imwrite(os.path.join("../output/issue", name), vis)
#                     print(f"💾 Saved large-motion frame: {name}")
#             except Exception as e:
#                 print(f"⚠️ Failed to save large motion frame: {e}")
...
# #   - 큰 움직임 감지 시 자동 프레임 저장 (../output/issue)
# # ==========================================================
# 📄 feature_tracker.py
# ------------------------------------------------------------
# Optical Flow (Sparse LK) 기반 특징점 추적 유틸리티 (Tracking-Loss 완화형)
# - Forward–Backward 체크를 완화하여 정상 이동률 향상
# - Laplacian blur 기준 완화 (약한 블러 프레임도 통과)
# - Adaptive Large Motion 필터로 개별 outlier 제거
# - 프레임 전체 큰 이동 감지 시 issue 프레임 저장 (../output/issue)
# ------------------------------------------------------------
